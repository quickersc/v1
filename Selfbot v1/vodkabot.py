...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + "Terima Kasih Telah Menambahkan Saya Sebagai Teman Chatmu")
...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    #print op
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + "Selamat Bergabung Di Grup" + group.name)
    except Exception as e:
        print e
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + "Mampus Dikick")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + "Selamat Tinggal Kawan")
    except Exception as e:
        print e
    ...
    try:
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if msg.text == "groupurl":
                    sendMessage(msg.to,"line://ti/g/" + client._client.reissueGroupTicket(msg.to))
                if msg.text == "groupopen":
                    group = client.getGroup(msg.to)
                ...
                if "kickon:" in msg.text:
                    ...
                    if key in Names:
                        ...
                        sendMessage(msg.to, "Apa-apaan nih kenapa begini ?")
                if msg.text == "cancel":
                    group = client.getGroup(msg.to)
                    if group.invitee is None:
                        sendMessage(op.message.to, "Tidak ada Undangan Yang Tertunda")
                    else:
                ...
                if "invite:" in msg.text:
                    ...
                    contact = client.getContact(key)
                    sendMessage(msg.to, ""+contact.displayName+"Saya Mengundangmu Untuk Bergabung")
                if msg.text == "me":
                    M = Message()
                    M.to = msg.to
                    M.contentType = 13
